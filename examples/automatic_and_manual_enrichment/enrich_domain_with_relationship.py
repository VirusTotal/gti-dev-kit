...
# display only the relationship data for a specified Domain. It helps security analysts 
...
#
# Output Summary:
# - Displays a well-structured summary of all threat relationships associated with the input Domain.
...
# - Replace the example Domain in the `main()` function with your target Domain.
# ========================================================================================
...
def get_relationship_cache_file(domain: str) -> str:
    """
    Returns the file path for the relationship cache of a given domain.
    ...
    Returns:
        str: Cache file path.
    """
    return os.path.join(CACHE_DIR, f"domain_{domain}_relationships_cache.json")
...
def make_api_request(url: str, params: Dict = None) -> Dict:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            result['error'] = "Bad request - invalid domain or parameters."
...
def get_domain_relationships(domain: str) -> Dict:
    """
    Fetches relationship data from GTI for a given domain, using local cache if available.
    ...
    """
    cache_file = get_relationship_cache_file(domain)

    if os.path.exists(cache_file):
        print(f"Found cached relationship data for {domain}")
        try:
    ...
    print(f"Fetching relationship data for Domain: {domain}")
...
def print_relationships(response: Dict, domain: str):
    """
    ...
    if not relationships:
        print(f"No relationships found for domain: {domain}")
        return

    print(f"\n=== Relationship Summary for Domain: {domain} ===")
...
def main():
    """
    Entry point for the script. Modify the domain as needed.
    """
    domain = "example.com"  # Replace with your target domain
    print(f"Analyzing Domain: {domain}")

    domain_relationships = get_domain_relationships(domain)
    if not domain_relationships['success'] and domain_relationships.get('should_retry'):
        print("\nRetrying failed request...")
        domain_relationships = get_domain_relationships(domain)
    print_relationships(domain_relationships, domain)
