# ======================================================================================================
# This script submits a URL to the Google Threat Intelligence (GTI) API for public analysis and retrieves
# a complete scan report. It is intended for developers, security analysts, and threat researchers who 
# need to evaluate URLs using GTI's public scanning infrastructure.
#
# Output Summary:
# - Displays scan status and progress with polling attempts
# - Prints the overall verdict (MALICIOUS or CLEAN)
# - Highlights GTI's contextual assessment of the URL
# - Provides the full analysis report as prettified JSON
#
# Usage:
# - Set your GTI_API_KEY and X_TOOL_HEADER at the top of the script.
# - Replace the default demo URL in the `main()` function with your target URL.
# - Run the script to scan and retrieve a detailed analysis report for the specified URL.
# ======================================================================================================
...
def submit_url_for_scanning(url: str) -> Dict[str, Any]:
    ...
    """
    payload = {"url": url}
    return make_api_request("POST", "urls", data=payload)
...
def get_url_report(url_id: str) -> Dict[str, Any]:
    ...
    """
    return make_api_request("GET", f"urls/{url_id}")

def scan_url_and_get_report(url: str) -> Optional[Dict]:
    """
    Scans a URL with comprehensive error handling.
...
def main():
    ...
    print(f"\nScanning URL: {url}")
    scan_result = scan_url_and_get_report(url)
