...
from examples.automatic_and_manual_enrichment.enrich_domain_with_relationship import (
...
def test_get_relationship_cache_file():
    ...
    expected = os.path.join(CACHE_DIR, f"domain_{DOMAIN}_relationships_cache.json")
    result = get_relationship_cache_file(DOMAIN)
...
def test_make_api_request_success(mock_requests_get):
    ...
    mock_response.json.return_value = ENRICH_DOMAIN_RELATIONSHIP_API_RESPONSE
    mock_requests_get.return_value = mock_response

    result = make_api_request(f"{BASE_URL}/domains/{DOMAIN}", params=PARAMS)

    assert result["success"] is True
    assert result["data"] == ENRICH_DOMAIN_RELATIONSHIP_API_RESPONSE
...
def test_make_api_request_400_bad_request(mock_requests_get):
    ...
    assert result["error"] == "Bad request - invalid domain or parameters."
...
def test_get_domain_relationships_from_cache(mock_os_path_exists, mock_open_file):
    """Test fetching relationship data from cache."""
    mock_os_path_exists.return_value = True
    mock_open_file().read.return_value = json.dumps(
        ENRICH_DOMAIN_RELATIONSHIP_API_RESPONSE
    )

    result = get_domain_relationships(DOMAIN)
...
def test_get_domain_relationships_cache_io_error(
...
def test_get_domain_relationships_no_cache_api_success(
    mock_os_path_exists,
    mock_requests_get,
    mock_open_file,
    mock_os_makedirs,
    ...
    mock_json_dump.assert_called_once_with(
        ENRICH_DOMAIN_RELATIONSHIP_API_RESPONSE, mock_open_file(), indent=2
    )


def test_get_domain_relationships_no_cache_cache_write_error(
...
def test_print_relationships_success(capsys):
    ...
    print_relationships(
        {"success": True, "data": ENRICH_DOMAIN_RELATIONSHIP_API_RESPONSE}, DOMAIN
    )
    captured = capsys.readouterr()

    assert f"Relationship Summary for Domain: {DOMAIN}" in captured.out
    assert "> Malware Families:" in captured.out
    assert (
        "- ID: analysis_virustotal_zenbox_metasploit | Type: collection" in captured.out
    )
    ...
    assert f"- ID: 23.192.228.80{DOMAIN} | Type: resolution" in captured.out
...
def test_print_relationships_failed_request(capsys):
    ...
    print_relationships(response, DOMAIN)
    captured = capsys.readouterr()
...
def test_print_relationships_no_relationships(capsys):
    ...
    assert f"No relationships found for domain: {DOMAIN}" in captured.out
...
def test_main_retry_logic(mock_os_path_exists, mock_requests_get, capsys):
    ...
    mock_response_success.json.return_value = ENRICH_DOMAIN_RELATIONSHIP_API_RESPONSE
    mock_requests_get.side_effect = [mock_response_fail, mock_response_success]

    with patch(
        "examples.automatic_and_manual_enrichment.enrich_domain_with_relationship.get_domain_relationships"
    ) as mock_get_domain_relationships:
        mock_get_domain_relationships.side_effect = [
            {"success": False, "error": "Rate limit exceeded.", "should_retry": True},
            {"success": True, "data": ENRICH_DOMAIN_RELATIONSHIP_API_RESPONSE},
        ]
        from examples.automatic_and_manual_enrichment.enrich_domain_with_relationship import (
            main,
        )
