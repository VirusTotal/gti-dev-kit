...
from examples.automatic_and_manual_enrichment.enrich_url_with_relationship import (
    get_cache_filename,
    make_api_request,
...
from testcases.constants import ENRICH_URL, ENRICH_URL_ID, ENRICH_URL_API_RESPONSE

MOCK_CACHE_FILE = os.path.join(
    CACHE_DIR, f"url_{ENRICH_URL_ID}_relationships_cache.json"
)
...
def test_get_cache_filename():
    ...
    expected = os.path.join(CACHE_DIR, f"url_{ENRICH_URL_ID}_relationships_cache.json")
    result = get_cache_filename(ENRICH_URL_ID)
...
def test_make_api_request_success(mock_requests_get):
    ...
    mock_response.json.return_value = ENRICH_URL_API_RESPONSE
    mock_requests_get.return_value = mock_response

    result = make_api_request(f"{BASE_URL}/urls/{ENRICH_URL_ID}", params=PARAMS)

    assert result["success"] is True
    assert result["data"] == ENRICH_URL_API_RESPONSE
    assert result["error"] is None
    assert result["status_code"] == 200
    assert result["should_retry"] is False
    mock_requests_get.assert_called_once_with(
        f"{BASE_URL}/urls/{ENRICH_URL_ID}",
...
def test_get_url_relationships_from_cache(mock_os_path_exists, mock_open_file):
    """Test fetching URL relationships from cache."""
    mock_os_path_exists.return_value = True
    mock_open_file().read.return_value = json.dumps(ENRICH_URL_API_RESPONSE)

    result = get_url_relationships(ENRICH_URL)
...
def test_get_url_relationships_cache_io_error(
...
def test_get_url_relationships_cache_json_error(
    mock_os_path_exists, mock_open_file, mock_requests_get
):
...
def test_get_url_relationships_no_cache_api_success(
    mock_os_path_exists,
    mock_requests_get,
    mock_open_file,
    mock_os_makedirs,
    ...
    """Test fetching URL relationships from API with successful cache save."""
    ...
    mock_json_dump.assert_called_once_with(
        ENRICH_URL_API_RESPONSE, mock_open_file(), indent=2
    )


def test_get_url_relationships_no_cache_cache_write_error(
...
def test_get_url_relationships_invalid_url():
    """Test URL relationships with invalid URL encoding."""
    invalid_url = "\ud800"
    result = get_url_relationships(invalid_url)
...
def test_print_relationships_success(capsys):
    """Test printing successful relationship data."""
    print_relationships({"success": True, "data": ENRICH_URL_API_RESPONSE}, ENRICH_URL)
    captured = capsys.readouterr()

    assert f"Relationship Summary for URL: {ENRICH_URL}" in captured.out
    assert "> Related Threat Actors:" in captured.out
    assert "- No related entities found." in captured.out
    assert "> Software Toolkits:" in captured.out
    assert "> Campaigns:" in captured.out
    assert "> Comments:" in captured.out
    assert f"- ID: u-{ENRICH_URL_ID}-231049ee | Type: comment" in captured.out
    assert "> Reports:" in captured.out
    assert (
        "- ID: report--815a1cc1c82742d68689ef6480d8f2ee2b385b3b288f2d890614e5522e492477 | Type: collection"
        in captured.out
    )
    assert "> Collections:" in captured.out
    assert (
        "- ID: 106db14ba76f8f141db690f2e0182146ea89a3a7d3850a14b7730dc932014aa5 | Type: collection"
        in captured.out
    )
    assert (
        "- ID: 212db69792038394b8ecdaf484fdf1260b839d97590acf7468d959f33578eefe | Type: collection"
        in captured.out
    )
    assert (
        "- ID: 21f77f0ed9ed633e1d2c54c3498daa404ef837caf3c63054cc9368aed6b206e0 | Type: collection"
        in captured.out
    )
    assert "> Malware Families:" in captured.out
    assert (
        "- ID: analysis_virustotal_zenbox_redlinestealer | Type: collection"
        in captured.out
    )
    assert "- ID: analysis_virustotal_zenbox_xworm | Type: collection" in captured.out
    assert "> Vulnerabilities:" in captured.out


def test_print_relationships_failed_request(capsys):
    """Test printing failed relationship request."""
    print_relationships(
        {"success": False, "error": "API error", "should_retry": True}, ENRICH_URL
    )
    captured = capsys.readouterr()
...
def test_print_relationships_no_data(capsys):
    """Test printing with no relationship data."""
    print_relationships({"success": True, "data": {}}, ENRICH_URL)
    captured = capsys.readouterr()

    assert f"No relationships found for URL: {ENRICH_URL}" in captured.out


def test_print_relationships_empty_relationships(capsys):
    """Test printing with empty relationships."""
    print_relationships(
        {"success": True, "data": {"data": {"relationships": {}}}}, ENRICH_URL
    )
    captured = capsys.readouterr()
...
def test_main_retry_logic(mock_os_path_exists, mock_requests_get, capsys):
    ...
    mock_response_success.json.return_value = ENRICH_URL_API_RESPONSE
    mock_requests_get.side_effect = [mock_response_fail, mock_response_success]

    with patch(
        "examples.automatic_and_manual_enrichment.enrich_url_with_relationship.get_url_relationships"
    ) as mock_get_url_relationships:
        mock_get_url_relationships.side_effect = [
            {"success": False, "error": "Rate limit exceeded.", "should_retry": True},
            {"success": True, "data": ENRICH_URL_API_RESPONSE},
        ]
        from examples.automatic_and_manual_enrichment.enrich_url_with_relationship import (
            main,
        )
    ...
    captured = capsys.readouterr()
    assert f"Analyzing URL: {ENRICH_URL}" in captured.out
    assert "Retrying failed request..." in captured.out
    assert f"Relationship Summary for URL: {ENRICH_URL}" in captured.out
    assert f"Analysis completed for: {ENRICH_URL}" in captured.out
    assert "> Comments:" in captured.out
    assert f"- ID: u-{ENRICH_URL_ID}-231049ee | Type: comment" in captured.out
    assert "> Malware Families:" in captured.out
    assert (
