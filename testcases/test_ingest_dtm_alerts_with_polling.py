...
from examples.dtm.ingest_dtm_alerts_with_polling import (
    make_api_request,
    list_dtm_alerts,
...
def test_make_api_request_success(mock_requests_get):
    ...
    mock_response.headers = {
        "link": '<https://api.intelligence.mandiant.com/v4/dtm/alerts?page=2>; rel="next"'
    }
...
def test_list_dtm_alerts_success(mock_requests_get):
    ...
    params = {"size": 100, "since": "2025-08-01T00:00:00Z", "page": "1"}
...
def test_parse_next_page_token_valid():
    """Test parsing valid next page token from headers."""
    headers = {
        "link": '<https://api.intelligence.mandiant.com/v4/dtm/alerts?page=2>; rel="next", <https://api.intelligence.mandiant.com/v4/dtm/alerts?page=1>; rel="prev"'
    }
    result = parse_next_page_token(headers)
    assert result == "2"


def test_parse_next_page_token_no_link():
    """Test parsing when no link header is present."""
    headers = {}
    result = parse_next_page_token(headers)
    assert result is None


def test_parse_next_page_token_no_next():
    """Test parsing when link header exists but no 'rel=next'."""
    headers = {
        "link": '<https://api.intelligence.mandiant.com/v4/dtm/alerts?page=1>; rel="prev"'
    }
    result = parse_next_page_token(headers)
    assert result is None


def test_parse_next_page_token_no_page():
    """Test parsing when link header has 'rel=next' but no page parameter."""
    headers = {
        "link": '<https://api.intelligence.mandiant.com/v4/dtm/alerts>; rel="next"'
    }
    result = parse_next_page_token(headers)
    assert result is None
...
def test_print_dtm_results_no_alerts(capsys):
    ...
    response = {"success": True, "data": {"alerts": []}, "headers": {}}
...
def test_print_dtm_results_formatting_error(capsys):
    ...
    response = {"success": True, "data": None, "headers": {}}
    result = print_dtm_results(response)
    captured = capsys.readouterr()
...
def test_poll_dtm_alerts_success(mock_requests_get, mock_time_sleep, capsys):
    """Test polling DTM alerts with successful responses."""
    mock_response = MagicMock(status_code=200)
    mock_response.json.return_value = DTM_ALERTS_API_RESPONSE
    mock_response.headers = {
        "link": '<https://api.intelligence.mandiant.com/v4/dtm/alerts?page=2>; rel="next"'
    ...
    poll_dtm_alerts(polls=2, interval=5)
    captured = capsys.readouterr()
    ...
    assert mock_requests_get.call_args_list[0][1]["params"] == {
        "refs": bool(False),
        "size": 100,
        "order": "desc",
        "since": pytest.approx(
    ...
    assert mock_requests_get.call_args_list[1][1]["params"] == {"page": "2"}


def test_poll_dtm_alerts_with_retry(mock_requests_get, mock_time_sleep, capsys):
    """Test polling DTM alerts with retry on failed request."""
...
def test_poll_dtm_alerts_no_alerts(mock_requests_get, mock_time_sleep, capsys):
    """Test polling DTM alerts with no alerts."""
    mock_response = MagicMock(status_code=200)
...
def test_poll_dtm_alerts_pagination(mock_requests_get, mock_time_sleep, capsys):
    """Test polling DTM alerts with pagination."""
    mock_response_1 = MagicMock(status_code=200)
    mock_response_1.json.return_value = DTM_ALERTS_API_RESPONSE
    mock_response_1.headers = {
        "link": '<https://api.intelligence.mandiant.com/v4/dtm/alerts?page=2>; rel="next"'
    }
    mock_response_2 = MagicMock(status_code=200)
    mock_response_2.json.return_value = {"alerts": []}
...
def test_main(mock_requests_get, mock_time_sleep, capsys):
    ...
    from examples.dtm.ingest_dtm_alerts_with_polling import main

    main()

    captured = capsys.readouterr()
    assert "Starting DTM Alerts Polling..." in captured.out
...
def test_main_with_retry(mock_requests_get, mock_time_sleep, capsys):
...
def test_main_no_alerts(mock_requests_get, mock_time_sleep, capsys):
