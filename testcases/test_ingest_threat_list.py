...
def test_print_threat_list_exception(monkeypatch, capsys):
    def mock_dumps(*args, **kwargs):
        raise ValueError("Mocked JSON error")

    monkeypatch.setattr("json.dumps", mock_dumps)

    response = {"success": True, "data": {"iocs": [{"id": 1, "threat": "malware"}]}}

    result = print_threat_list(response, "malware")
    captured = capsys.readouterr()

    assert result is False
    assert "Error processing threat list: Mocked JSON error" in captured.out
