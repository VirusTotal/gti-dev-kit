...
from examples.file_and_url_scanning.private_scanning.scan_url import ( 
    make_api_request,
    submit_url_for_scanning,
    poll_analysis_status,
    get_url_report,
    scan_private_url_and_get_report,
    print_scan_report,
    POLLING_INTERVAL,
...
@patch("examples.file_and_url_scanning.private_scanning.scan_url.make_api_request")
def test_submit_url_for_scanning_success(mock_make_request):
    mock_make_request.return_value = {
        "success": True,
        "data": SCAN_PRIVATE_URL_RESPONSE,
    }
    result = submit_url_for_scanning("https://example.com")
    assert result["success"] is True
    assert result["data"] == SCAN_PRIVATE_URL_RESPONSE

    args, kwargs = mock_make_request.call_args
    assert args[0] == "POST"
    assert args[1] == "private/urls"
...
def test_poll_analysis_status_completed(mock_make_request):
    mock_make_request.return_value = {
        "success": True,
        "data": SCAN_PRIVATE_URL_ANALYSIS_RESPONSE,
    }
...
def test_get_url_report_success(mock_make_request):
    mock_make_request.return_value = {
        "success": True,
        "data": SCAN_PRIVATE_URL_REPORT_RESPONSE,
    }
    result = get_url_report("test_url_id")
    assert result["success"] is True
    assert result["data"] == SCAN_PRIVATE_URL_REPORT_RESPONSE


@patch("examples.file_and_url_scanning.private_scanning.scan_url.get_url_report")
@patch("examples.file_and_url_scanning.private_scanning.scan_url.poll_analysis_status")
@patch(
    "examples.file_and_url_scanning.private_scanning.scan_url.submit_url_for_scanning"
)
def test_scan_private_url_success(mock_submit, mock_poll, mock_report):
    mock_submit.return_value = {"success": True, "data": SCAN_PRIVATE_URL_RESPONSE}
    ...
    result = scan_private_url_and_get_report("https://example.com")
    assert result == SCAN_PRIVATE_URL_REPORT_RESPONSE
    mock_submit.assert_called_once_with("https://example.com")
    mock_poll.assert_called_once_with(SCAN_PRIVATE_URL_RESPONSE["data"]["id"])
...
def test_scan_private_url_submit_failure(mock_submit):
...
def test_scan_private_url_submit_retry_success(mock_submit):
    mock_submit.side_effect = [
        {"success": False, "error": "Temp error", "should_retry": True},
        {"success": True, "data": SCAN_PRIVATE_URL_RESPONSE},
    ]

    with patch(
        "examples.file_and_url_scanning.private_scanning.scan_url.poll_analysis_status"
    ):
        with patch(
            "examples.file_and_url_scanning.private_scanning.scan_url.get_url_report"
        ):
            result = scan_private_url_and_get_report("https://example.com")
...
def test_print_scan_report_valid(capsys):
    print_scan_report(SCAN_PRIVATE_URL_REPORT_RESPONSE)
